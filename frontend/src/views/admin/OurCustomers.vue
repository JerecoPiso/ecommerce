    ...
    <div class="row">
        <div class="col-xl-12">
            <div class="tables mt-1">
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        <tbody>
                            <tr v-for="v in verifiedUsers" :key="v.id">
                                <td><img :src="'/images/49f05c5d08bfaa7bd12415cf1dda90e3.ico'" alt="" class="user-dp">
                                    &ThinSpace; <span>{{ v.username }}</span> </td>
                                <td>{{ v.email }}</td>
                ...
                <TablePagination :current-page="currentPageV" :total-page="totalPageV" @next="nextPageV()"
                    @prev="prevPageV()"></TablePagination>
            </div>
        </div>
        <div class="col-xl-12">
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        <tbody>
                            <tr v-for="v in unverifiedUsers" :key="v.id">
                                ...
                                <td>{{ v.email }}</td>
                                <td><span :class="setClass(v.status)">{{ setStatus(v.status) }}</span></td>
                                <td class="action-max-width"><button type="button" class="btn-verify">Verify</button><button
                                        type="button" class="btn-decline">Decline</button></td>

                            </tr>
                        </tbody>
                    </table>
                </div>
                <TablePagination :current-page="currentPageUV" :total-page="totalPageUV" @next="nextPageUV()"
                    @prev="prevPageUV()"></TablePagination>
            </div>
        </div>
    </div>
</template>
