<template>
            ...
            <div class="table-products">
                <button type="button"  data-bs-toggle="modal" data-bs-target="#myModal" class="add-btn"> <i class="fa-solid fa-plus"></i> Add Product</button>
                    ...
                    <tbody>
                      <tr v-for="product, index in products" :key="product.id">
                          ...
                          <td class="action-max-width">
                            <button class="action-btn btn-add mt-2"  @click="addStock(product.id, product.stocks)"><i class="fa-solid fa-plus"></i> </button> &ThinSpace;
                            <button class="action-btn btn-edit"><i class="fa-solid fa-pen-to-square"></i> </button> &ThinSpace;
                            <button class="action-btn btn-archive" @click="archiveProduct(product.id, index, 1)"><i class="fa-solid fa-trash"></i> </button>
                          </td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                <TablePagination :current-page="currentPage" :total-page="totalPage" @next="nextPage()" @prev="prevPage()"></TablePagination>
            </div>
            <!-- .table-products -->
            <div class="row">
              <div class="col-xl-6 col-lg-8">
                <div class="tables">
                  ...
                  <div class="tbl-max-height">
                     <div class="sold-out" v-for="product, index in outofstock" :key="product.id">
                        <p class="mb-2 ff"><img :src="'/images/'+product.img_name" class="tbl-product-img"><span class="ms-4">{{product.product_name}}</span></p>   
                        <div class="btns">
                ...
                <div class="col-xl-3 col-lg-4">
                  <div class="tables">
                    <p class="table-title">Product Category <button type="button" class="btn-add-category" @click="addCategory()"> <i class="fa-solid fa-plus"></i></button> </p>
                    <div class="tbl-max-height ">
                      <table class="table table-hover table-bordered">
                        <tbody>
                          <tr v-for="c, index in category" :key="c.id">
                             <td>{{c.category}}</td>
                             <td class="action-td">
                              <button class="action-btn btn-edit" @click="updateCategory(c.id, c.category, index)"><i class="fa-solid fa-pen-to-square"></i> </button> &ThinSpace;
                              <button class="action-btn btn-archive " @click="archiveCategory(c.id, index)"><i class="fa-solid fa-trash"></i> </button>
                            </td>
                          </tr>
                        </tbody>
                      </table>
                ...
                <div class="col-xl-3 col-lg-4">
                  <div class="tables">
                    <p class="table-title">Product Brand <button type="button" class="btn-add-brand" @click="addBrand()"> <i class="fa-solid fa-plus"></i> </button></p>
                    <div class="tbl-max-height">
                      <table class="table table-hover table-bordered">
                        <tbody>
                          <tr v-for="b, index in brands" :key="b.id">
                            ...
                            <td class="action-td" style="width: 40%;">
                              <button class="action-btn btn-edit" @click="updateBrand(b.id, b.brand, index)"><i class="fa-solid fa-pen-to-square"></i> </button> &ThinSpace;
                              <button class="action-btn btn-archive " @click="archiveBrand(b.id, index)"><i class="fa-solid fa-trash"></i> </button>
                            </td>
                          </tr>
                        </tbody>
                      </table>
...
export default{
    ...
    methods:{
        ...
        async AddProduct(){ 
                await axios.post(this.url+"admin/createProduct", this.productInfo, { headers: {
